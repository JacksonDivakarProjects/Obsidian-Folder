...
WHEN ISNULL(customer_ltm, 0) = 0

AND ISNULL(customer_prev_ltm, 0) <> 0

THEN 'Yes'

ELSE 'No'

END AS customerchurn_flag,

CASE

WHEN ISNULL(customer_ytd, 0)= 0

AND ISNULL(customer_prev_ytd, 0)<> 0

THEN 'Yes'

ELSE 'No'

END AS customerchurn_flag_ytd
...
isnull(customer_key, '')customer_key,
...
when x.range_flag = 'No'

and x.end_flag = 'No' then 'Yes'

else 'No'

end as cust_churn_flag
...
when d.customer_enddt BETWEEN DATEADD(MONTH, -11, t.month_roll) AND t.month_roll then 'Yes'
...
when d.customer_enddt > t.month_roll then 'Yes'
...
isnull(t.customer_key, '') = isnull(d.customer_key, '')
...
when d.customer_enddt BETWEEN DATEFROMPARTS(YEAR(t.month_roll), 1, 1) AND t.month_roll then 'Yes'
...
c.customerchurn_flag = s.cust_churn_flag

FROM

#cte_customerchurn c

inner join #customerchurn_startdt s on

c.month_roll = s.month_roll
...
customerchurn_flag_ytd = s.cust_churn_flag
...
WHEN c.customerchurn_flag = 'Yes'

THEN CAST(-t.bop_arr AS FLOAT)
...
WHEN c.customerchurn_flag_ytd = 'Yes'

THEN CAST(-t.bop_ytd AS FLOAT)
...
WHEN ISNULL(cust_ltm, 0) <> 0

AND ISNULL(cust_prev_ltm, 0) = 0

THEN 'Yes'

ELSE 'No'

END AS new_cust_flag,

CASE

WHEN ISNULL(cust_ytd, 0) <> 0

AND ISNULL(cust_prev_ytd, 0) = 0

THEN 'Yes'

ELSE 'No'

END AS new_cust_flag_ytd
...
WHEN customer_churn = 0

THEN eop_arr

ELSE 0

END) AS FLOAT) AS cust_ltm,
...
END) AS FLOAT) AS cust_prev_ltm,

CAST(SUM(CASE

WHEN customer_churn_ytd = 0

THEN eop_ytd

ELSE 0

END) AS FLOAT) AS cust_ytd,
...
BETWEEN DATEADD(MONTH, -11, t.month_roll)

AND t.month_roll

and data_source = 'Sage'
...
BETWEEN DATEFROMPARTS(YEAR(t.month_roll), 1, 1)
...
isnull(s.customer_key, '')= isnull(c.customer_key, '')
...
when new_cust_flag = 'Yes'

then cast(t.eop_arr as float)
...
WHEN new_cust_flag_ytd = 'Yes'

THEN CAST(t.eop_ytd AS FLOAT)
...
WHEN ISNULL(plan_ltm, 0) = 0

AND ISNULL(plan_prev_ltm, 0) <> 0

THEN 'Yes'

ELSE 'No'

END AS planchurn_flag,

CASE

WHEN ISNULL(plan_ytd, 0)= 0

AND ISNULL(plan_prev_ytd, 0)<> 0

THEN 'Yes'

ELSE 'No'

END AS planchurn_flag_ytd
...
END) AS FLOAT) AS plan_prev_ltm,

CAST(SUM(CASE

WHEN customer_churn_ytd = 0

AND new_customer_ytd = 0

THEN eop_ytd

ELSE 0

END) AS FLOAT) AS plan_ytd,
...
when d.plan_enddt BETWEEN DATEADD(MONTH, -11, t.month_roll) AND t.month_roll then 'Yes'
...
when d.plan_enddt > t.month_roll then 'Yes'
...
when d.plan_enddt BETWEEN DATEFROMPARTS(YEAR(t.month_roll), 1, 1) AND t.month_roll then 'Yes'
...
c.planchurn_flag = s.plan_churn_flag

FROM

#cte_planchurn c

inner join #planchurn_startdt s on
...
c.planchurn_flag_ytd = s.plan_churn_flag
...
and ISNULL(plan_prev_ltm, 0) = 0 THEN 'Yes'
...
d.plan_startdt BETWEEN DATEADD(MONTH, -11, t.month_roll) AND t.month_roll
...
d.plan_startdt BETWEEN DATEFROMPARTS(YEAR(t.month_roll), 1, 1) AND t.month_roll
...
inner join #cte_cross_sell c on
...
WHEN ISNULL(service_ltm, 0) <> 0

AND ISNULL(service_prev_ltm, 0) = 0

THEN 'Yes'

ELSE 'No'

END AS service_cross_sell_flag,

CASE

WHEN ISNULL(service_ytd, 0) <> 0

AND ISNULL(service_prev_ytd, 0) = 0

THEN 'Yes'

ELSE 'No'

END AS service_cross_sell_flag_ytd

INTO

#cte_service_cross_sell
...
END) AS FLOAT) AS service_prev_ltm,
...
AND cross_sell_ytd = 0

THEN eop_ytd

ELSE 0

END) AS FLOAT) AS service_ytd,
...
END) AS FLOAT) AS service_prev_ytd
...
t.service_key,t.service_dim_key,
...
d.service_startdt BETWEEN DATEADD(MONTH, -11, t.month_roll) AND t.month_roll
...
service_cross_sell_flag = 'Yes'

FROM

#cte_service_cross_sell c
...
service_cross_sell_flag_ytd = 'Yes'
...
service_cross_sell_flag_ytd = 'No'
...
when service_cross_sell_flag = 'Yes'
...
inner join #cte_service_cross_sell c on
...
service_cross_sell_ytd =

CASE

WHEN service_cross_sell_flag_ytd = 'Yes'
...
x.service_key,x.service_dim_key,
...
when d.service_enddt BETWEEN DATEADD(MONTH, -11, t.month_roll) AND t.month_roll then 'Yes'
...
when d.service_enddt > t.month_roll then 'Yes'
...
WHEN d.service_enddt BETWEEN DATEFROMPARTS(YEAR(t.month_roll), 1, 1)
...
WHEN d.service_enddt > t.month_roll
...
c.servicechurn_flag = s.service_churn_flag

FROM

#cte_servicechurn c

inner join #servicechurn_startdt s on
...
c.servicechurn_flag_ytd = s.service_churn_flag
...
WHEN c.servicechurn_flag_ytd = 'Yes'
...
WHEN isnull(service_ltm, 0) < isnull(service_prev_ltm, 0)
...
WHEN ISNULL(service_ytd, 0) < ISNULL(service_prev_ytd, 0)

THEN 'Yes'

ELSE 'No'

END AS downsell_flag_ytd
...
WHEN servicechurn_ytd = 0

AND customer_churn_ytd = 0
...
when downsell_flag = 'Yes'

then cast(t.eop_arr as float)-cast(t.bop_arr as float)
...
WHEN downsell_flag_ytd = 'Yes'

THEN CAST(t.eop_ytd AS FLOAT) - CAST(t.bop_ytd AS FLOAT)
...
int_cust.revenue_bridge_output_bucket_calculations
...
--select * from revenue_bridge_output_bucket_calculations
...
grr = bop_arr + customer_churn + planchurn + servicechurn + downsell

from

int_cust.revenue_bridge_output_bucket_calculations a;
...
nrr = grr + upsell + service_cross_sell + cross_sell
...
grr_ytd =bop_ytd + customer_churn_ytd + planchurn_ytd + servicechurn_ytd + downsell_ytd
...
nrr_ytd = grr_ytd + upsell_ytd + service_cross_sell_ytd + cross_sell_ytd
