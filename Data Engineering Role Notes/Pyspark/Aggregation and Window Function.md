...
Aggregations and window functions are where PySpark moves from simple row manipulation to real analytical processing.
...
# Create a detailed sample DataFrame
data = [
...
**Concept:** Split data into groups by one or more columns, then compute summary statistics per group.

**Practical uses:** reports, departmental KPIs, understanding distributions, ML feature engineering.
...
# Multiple grouping columns: Department and State
...
# Multiple aggregate functions on different columns
...
**Concept:** Compute a value across a set of rows related to the current row, **without collapsing rows** — unlike `groupBy()`, window functions add new columns to every original row.

**Practical uses:** rankings, running totals, moving averages, comparing a row to its group's average.
...
# Running totals within departments by hire date
