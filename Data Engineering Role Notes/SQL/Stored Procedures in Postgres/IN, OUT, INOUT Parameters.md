...
This is the core distinction to internalize: `IN` is read-only inside the procedure, `OUT` is write-only and starts NULL, and `INOUT` is both — it arrives with the caller's value and leaves with whatever the procedure last assigned to it.

## 🔗 Related Notes
- [[Stored Procedures Notes|Comprehensive Beginner's Guide to PostgreSQL Stored Procedures]]
- [[Question & Answers of SP|Question & Answers of SP]]
