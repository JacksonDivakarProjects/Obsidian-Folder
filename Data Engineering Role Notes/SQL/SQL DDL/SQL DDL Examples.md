...
Common `CREATE`/`ALTER TABLE` patterns: default constraints, foreign keys and their delete behavior, check constraints, indexes, and numeric data types. Each example includes the SQL and a short explanation of what it does.
...
Note the difference from example 2: `ON DELETE SET NULL` keeps the child row but nulls the foreign key; `ON DELETE CASCADE` deletes the child row outright. Pick `CASCADE` only when the child truly has no meaning without its parent (order items without an order), and `SET NULL`/`RESTRICT` when it might.

## 🔗 Related Notes
- [[Working of Index in SQL|Working of Index in SQL]]
- [[Basic SQL Optimizations|Basic SQL Optimizations]]
