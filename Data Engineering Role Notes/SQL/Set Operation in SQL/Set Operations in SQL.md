...
```sql
-- Find employees who are also someone's manager
...
Both sides return a single `employee_id`-shaped column, which is what `INTERSECT` requires — matching column count and compatible types on each side, regardless of which table they come from.
...
1. **Prefer `UNION ALL`** whenever duplicates are known to be impossible, or are acceptable — it skips the deduplication pass that plain `UNION` always pays for.
2. **Index the columns** used in the set operation, especially on large tables.
3. **Consider `EXISTS`/`NOT EXISTS`** as alternatives to `INTERSECT`/`EXCEPT` for better performance on large datasets.
4. **Keep column order (and count and type) consistent** across every query in the set operation — this is a hard requirement, not just a style preference.
5. **Use CTEs** to keep complex, multi-step set operations readable.
6. **Test on small datasets first** before running against production-sized data.
7. **Know your database's syntax** — `EXCEPT` (SQL Server, PostgreSQL) vs. `MINUS` (Oracle) is the most common divergence.

## 🔗 Related Notes
- [[SQL SubQueries|Comprehensive Guide to SQL Subqueries]]
- [[Recursive CTE Notes|Comprehensive Recursive CTE Guide]]
