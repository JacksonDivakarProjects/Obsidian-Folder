...
3. **Roll-up aggregation** — keep `root_id` constant through the recursion, aggregate at the end
4. **Path building** — string concatenation in the recursive term
5. **Leaf detection** — find nodes that never appear as someone else's manager
6. **Cycle detection** — track visited nodes in an array and check `id = ANY(path)`

These 6 patterns cover the bulk of the recursive-CTE questions companies actually ask.

## 🔗 Related Notes
- [[Recursive CTE Notes|Comprehensive Recursive CTE Guide]]
- [[Recursive CTE problem Statements|Recursive CTE problem Statements]]
