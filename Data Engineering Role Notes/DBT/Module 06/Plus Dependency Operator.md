...
For the related but distinct `@` operator (which also pulls in the ancestors of a model's children — used for safely rebuilding ephemeral-model dependents) and `state:modified+`, see [[dbt Node Selection Syntax|dbt Node Selection Syntax]].
