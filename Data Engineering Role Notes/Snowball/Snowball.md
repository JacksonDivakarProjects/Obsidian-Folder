...
- [[ARR Bridge Course - Chapter 4|Chapter 4 — Beyond SaaS: A Real Production Spark Build]] — a five-lesson escalation modeled on a real production PySpark pipeline (genericized as fictional Thornfield Veterinary Group): the grace-period fix for the LTM note's "blip" problem, a three-tier lapse/churn/death classification, the tall `period_type` reporting pattern with point-in-time dimension joins, and production-grade reconciliation.
