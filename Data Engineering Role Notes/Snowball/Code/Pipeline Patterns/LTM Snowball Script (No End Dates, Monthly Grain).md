...
If that blip rate turns out to matter for your business, the standard fix is a **grace period**: don't call it churn on the first missing month, only after N consecutive missing months (commonly 2 or 3, matched to your typical invoice/payment-retry cycle). That needs a running "months since this grain was last seen" calculation ahead of `customer_flags`, which is a genuine extension, not a one-line tweak — deliberately left out of this note so the core pattern stays the thing you actually learn first. [[ARR Bridge Course - Chapter 4|Chapter 4]] builds this fix for real, as a second rolling window rather than a running counter — see [[Lesson 2 - The Grace Period Problem, Solved|Lesson 2]].
